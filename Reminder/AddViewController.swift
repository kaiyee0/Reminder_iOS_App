...
class AddViewController: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let mydb = db {
            ...
            myTextField.attributedPlaceholder = NSAttributedString(string: "Todos Title", attributes: [NSForegroundColorAttributeName: UIColor.lightGray])
            ...
            //story input
            let mytextView = UITextView(frame: CGRect(x: 0.0, y: height * 1.6, width: fullsize.width, height: height*5))
            mytextView.keyboardType = .default
            mytextView.backgroundColor = UIColor.init(red: 0.05, green: 0.05, blue: 0.05, alpha: 1)
            mytextView.textAlignment = .center
            mytextView.textColor = UIColor.white
            mytextView.font = UIFont(name: "Helvetica Light", size: 24.0)
            //            mytextView.attributedPlaceholder = NSAttributedString(string: "Story...", attributes: [NSForegroundColorAttributeName: UIColor.lightGray])
            //            mytextView.rightView = UIView(frame: CGRect(x: 0, y: 0, width: padding, height: padding))
            //            mytextView.rightViewMode = .always
            mytextView.tag = 102
            //            mytextView.returnKeyType = .next
            ...
            // date input
            myTextField = UITextField(frame: CGRect(x: 0.0, y: height * 7.5, width: fullsize.width, height: height))
            myTextField.backgroundColor = UIColor.init(red: 0.05, green: 0.05, blue: 0.05, alpha: 1)
            myTextField.textAlignment = .center
            myTextField.textColor = UIColor.white
            ...
            let cancelBtn = UIBarButtonItem(title: "Cancel", style: .plain, target: self, action: #selector(AddViewController.cancelTouched(_:)))
            let space = UIBarButtonItem(barButtonSystemItem: .flexibleSpace, target: self, action: nil)
            let doneBtn = UIBarButtonItem(title: "Done", style: .plain, target: self, action: #selector(AddViewController.doneTouched(_:)))
            toolBar.items = [cancelBtn, space, doneBtn]
            myTextField.inputAccessoryView = toolBar
            
            // save btn
            let saveBtn = UIButton(frame: CGRect(x: fullsize.width * 0.25, y: height * 9, width: fullsize.width * 0.5, height: height * 1.5))
            ...
            // delete btn
            if record.id != 0 {
                let deleteBtn = UIButton(frame: CGRect(x: 5.0, y: height * 5.8, width: 100, height: height))
    ...
    func saveBtnAction() {
        ...
        let textView = self.view.viewWithTag(102) as! UITextView
        record.storyContent = textView.text ?? ""
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        let storyContent = self.view.viewWithTag(102) as! UITextView
